...
export const ActiveWorkout: React.FC<ActiveWorkoutProps> = ({
  ...
  return <div className="min-h-screen bg-background text-foreground flex flex-col animate-slide-in-right">
      {/* Sticky Header */}
      <div className="sticky top-0 z-10 bg-card/95 backdrop-blur-sm border-b border-border">
        <div className="flex justify-between items-center p-6 bg-gradient-to-r from-primary/10 to-primary/5">
          <div className="flex items-center space-x-3">
            <button onClick={onBack} className="bg-primary/10 text-primary p-2 rounded-xl border border-primary/20 hover:bg-primary/20 transition-colors">
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 mt-1">
                ...
                <span className="text-muted-foreground text-sm">{formatDate(workout.startTime)}</span>
          ...
          <button onClick={handleEndWorkout} className="bg-green-500/20 text-green-600 dark:text-green-400 p-2 rounded-xl border border-green-500/30 flex items-center space-x-2 hover:bg-green-500/30 transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-20">
        ...
        <div className="mt-4 mb-6">
          <div className="bg-card backdrop-blur-sm rounded-2xl p-6 border border-border shadow-sm">
            ...
            <div className="text-center mb-4">
              <div className="text-3xl font-bold text-primary">{formatDuration(duration)}</div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center">
                <div className="text-xl font-bold text-blue-600 dark:text-blue-400">{workout.totalSets}</div>
                <div className="text-xs text-muted-foreground">Total Sets</div>
              </div>
              <div className="text-center">
                <div className="text-xl font-bold text-green-600 dark:text-green-400">{formatTotalWeight(workout.totalWeight)}</div>
        ...
        <div className="space-y-3">
          {workout.exercises.map(exercise => <div key={exercise.id} ref={el => exerciseRefs.current[exercise.id] = el} className="bg-card backdrop-blur-sm rounded-xl p-3 border border-border shadow-sm animate-scale-in relative">
              {/* Exercise delete button */}
              <button onClick={() => openDeleteDialog('exercise', exercise.id)} className="absolute top-2 right-2 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 p-1">
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="space-y-2 mb-3">
                {exercise.sets.map((set, index) => <div key={set.id} ref={el => setRefs.current[set.id] = el} className="bg-secondary/50 rounded-lg p-3 relative">
                    {/* Set delete button */}
                    <button onClick={() => openDeleteDialog('set', exercise.id, set.id)} className="absolute top-2 right-2 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 p-1">
                      <X className="w-3 h-3" />
                    </button>
              ...
              {/* Add Set Button */}
              <button onClick={() => addSet(exercise.id)} className="w-full bg-primary/10 text-primary border border-primary/20 rounded-lg p-2.5 flex items-center justify-center space-x-2 hover:bg-primary/20 transition-colors">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-3 bg-card/95 backdrop-blur-sm border-t border-border">
        <button onClick={onAddExercise} className="w-full bg-gradient-to-r from-primary to-primary/80 text-primary-foreground rounded-2xl p-3 shadow-lg transform transition-all duration-200 hover:scale-105 active:scale-95">
