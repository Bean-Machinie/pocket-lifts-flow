...
export const MainDashboard: React.FC<MainDashboardProps> = ({ 
  ...
  const openDeleteDialog = (workoutId: string, isActive: boolean = false) => {
    setDeleteDialog({ isOpen: true, workoutId, isActive });
  };
  ...
  return (
    <div className="min-h-screen text-white p-6 animate-fade-in">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-3">
            {activeWorkouts.map((workout) => (
              <div
                key={workout.id}
                className="w-full bg-green-500/20 backdrop-blur-sm rounded-2xl p-4 border border-green-400/30 transform transition-all duration-200 hover:bg-green-500/25 hover:scale-[1.02] relative"
              >
                {/* Delete button */}
                <button
                  onClick={(e) => {
                ...
                >
                  ...
                  <div className="flex space-x-2 ml-6">
                    {workout.exercises.slice(0, 3).map((exercise, index) => (
                      <span
                        key={index}
                        className="bg-green-600/30 text-green-200 px-2 py-1 rounded-lg text-xs"
                      >
                        {exercise.name}
                      </span>
                    ))}
                    {workout.exercises.length > 3 && (
                      <span className="text-green-300 text-xs py-1">
      ...
      <DeleteConfirmDialog
        ...
        title={deleteDialog.isActive ? "Delete Active Workout" : "Delete Workout"}
        message={deleteDialog.isActive ? 
          "Are you sure you want to delete this active workout? This action cannot be undone." :
          "Are you sure you want to delete this workout? This action cannot be undone."
        }
        confirmText={deleteDialog.isActive ? "Delete Active Workout" : "Delete Workout"}
      />
